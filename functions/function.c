...
char* EMSCRIPTEN_KEEPALIVE getCreatureJSON(creature* cr, char* output) {
	creature* cur = cr;
	int i = 0;
	char obj[1024];
	snprintf(obj, 1024, "{\"creatures\":[");
	memcpy(&output[i], &obj, strlen(obj));
	i+=strlen(obj);
	while(cur != NULL) {
		snprintf(obj, 1024, 
				"{\"cid\":\"%d\","
				...
				"\"type\":\"%d\","
				"\"child\":\"%p\"},",
				cur->cId, cur->nrg, cur->xloc, cur->yloc, cur->zloc, cur->type, cur->child);
		memcpy(&output[i], &obj, strlen(obj));
	//	printf("%s\n",obj);
		i+=strlen(obj);
		cur = cur->child;
...
/**************************************************
*********** More JSON testing fuctions ************
**************************************************/

char* EMSCRIPTEN_KEEPALIVE buildJSON(int size){
...
float* EMSCRIPTEN_KEEPALIVE getNrgPtr(creature* cr){ return &cr->nrg;}
float* EMSCRIPTEN_KEEPALIVE getXlocPtr(creature* cr){ return &cr->xloc;}
float* EMSCRIPTEN_KEEPALIVE getYlocPtr(creature* cr){ return &cr->yloc;}
float* EMSCRIPTEN_KEEPALIVE getZlocPtr(creature* cr){ return &cr->zloc;}
uint32_t* EMSCRIPTEN_KEEPALIVE getCIdPtr(creature* cr){ return &cr->cId;}
uint8_t* EMSCRIPTEN_KEEPALIVE getTypePtr(creature* cr){ return &cr->type;}
creature* EMSCRIPTEN_KEEPALIVE getChildPtr(creature* cr){ return cr->child;}
